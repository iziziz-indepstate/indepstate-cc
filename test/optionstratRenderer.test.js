...
async function run() {
  ...
  assert.strictEqual(t.ticketBinding.resolveTicketKey('deal-open'), key);
  assert.strictEqual(t.placedOrderLookup.getPlacedOrder(key).ticket, 'deal-open');
  assert.strictEqual(t.placedOrderLookup.getPlacedOrder(key).payoff, payoff);
  assert.strictEqual(row.payoff, payoff);
  assert(Number.isFinite(row.openedAt));
  assert.strictEqual(t.cardVisualState.getCardState(key), 'placed');

  t.placedOrderLookup.markPlacedOrder(key, { provider: 'optionstrat', ticket: 'deal-1', symbol: 'SPY', name: row.name, strategyCommand: 'lcs', payoff });
  t.ticketBinding.bindTicket('deal-1', key);
  ...
  assert.strictEqual(t.placedOrderLookup.getPlacedOrder(key), undefined);
